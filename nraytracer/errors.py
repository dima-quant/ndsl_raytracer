...
def relative_error[T: SomeFloat](y: T, y_true: T) -> T:
    """{.inline.}"""
    ...
    with let: denom = max(abs(y_true), abs(y))
    if denom == cast[T](0):
      return cast[T](0)
    result = abs(y_true - y) / denom
    return result

def absolute_error[T: SomeFloat](y: T, y_true: T) -> T:
    """{.inline.}"""
...
@template
def ensureWithinRelTol[T: SomeFloat](y: T, y_true: T, tol = Tolerance) -> untyped:
...
@template
def ensureWithinAbsTol[T: SomeFloat](y: T, y_true: T, tol = Tolerance) -> untyped:
  ## If asserts, check that a value `y` is approximately `y_true`
